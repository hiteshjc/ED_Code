				...
				zMatrix &dmsum)
{
	...
	for(int64_t i = 0; i < dm11.NRows(); i++)
		...
		{
			dmsum(i,j) = conj(c1)*c1*dm11(i,j) + conj(c1)*c2*dm12(i,j) + conj(c2)*c1*conj(dm12(j,i)) + conj(c2)*c2*dm22(i,j);	
		}
}
...
//		statej = findstate_psi(big_stringj, psi1);
		...
		for(int64_t k = 0; k < n_dm; k++)
		{
			...
			//	dm(k, j) += psi1[statej].coeff*conj(psi2[statek].coeff);
//			if(j != k)
