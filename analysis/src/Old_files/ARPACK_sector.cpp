		      		...
		      		ofstream &outfile)
{
	...
	else
	{
		...
		for (int i=0; i<nev; i++)
			 for (int64_t j=0; j<n; j++) Evecs(i,j) = v[i*n+j];
		delete resid;
		delete v;
		delete d;
		delete iparam;
			...
			ofstream &outfile)
{
	...
	if (n_p > n_ev + 1)
	{
		...
		ARPACK_Iteration_EV(	pblock_states, Norm, n_spins, it, px, py,
				...
				ofstream &outfile)
{
	...
	ARPACK_sector(	pblock_states, Norm,
			pblock_eigs, pblock_eigvecs, find_ev,
			lambda, n_spins, px, py, n_ones, n_p,
			adj_list, Ch, ch_list, T1, T2, n_sec_ev, tol, maxiter, outfile);
