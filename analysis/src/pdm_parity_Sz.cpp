...
int main(int argc, char *argv[])
{
	...
	read_n_unwrap(filein, filepEV3, T1, T2, psi3, outfile);
	...
	outfile << " Size of unwrapped EV2: " << psi2.size() << endl;
	outfile << " Size of unwrapped EV3: " << psi3.size() << endl;
	outfile.flush();
...
//[-1 1 ]
	...
	outfile << "\n\n Dot Product for vectors 1 and 1 (both parity 1)" << dot_product_ids(psi1,psi1) << endl << endl;
	outfile << "\n\n Dot Product for vectors 1 and 2 (both parity 1)" << dot_product_ids(psi1,psi3) << endl << endl;
	outfile << "\n\n Dot Product for vectors 2 and 2 (both parity 1)" << dot_product_ids(psi3,psi3) << endl << endl;
...
//	{
		...
		if (ev1==1 and ev2==2) density_matrixij_Sz(denm, psi1, psi3, Cut, n_sites, n_ones, outfile);
		if (ev1==2 and ev2==1) density_matrixij_Sz(denm, psi3, psi1, Cut, n_sites, n_ones, outfile);
		if (ev1==2 and ev2==2) density_matrixij_Sz(denm, psi3, psi3, Cut, n_sites, n_ones, outfile);
