...
class Matrix4 : public std::vector<double>
{
	int64_t m;	int64_t n;	int64_t o;	int64_t p;

public:
	Matrix4() : m(0), n(0), o(0), p(0) {}

	Matrix4(int64_t m_, int64_t n_, int64_t o_, int64_t p_) : std::vector<double>(m_*n_*o_*p_), m(m_), n(n_), o(o_), p(p_) {}

	Matrix4(int64_t m_, int64_t n_, int64_t o_, int64_t p_, double val) : std::vector<double>(m_*n_*o_*p_, val), m(m_), n(n_), o(o_), p(p_) {}

	void resize(int64_t m_, int64_t n_, int64_t o_, int64_t p_)
	{	m = m_; n = n_; o = o_; p = p_; std::vector<double>::resize(m_*n_*o_*p_);}

	void reserve(int64_t m_, int64_t n_, int64_t o_, int64_t p_)
	{	std::vector<double>::reserve(m_*n_*o_*p_);}

	void clear()
	{	m = n = o = p = 0; std::vector<double>::clear();}

	int64_t N1() const {return m;}
	int64_t N2() const {return n;}
	int64_t N3() const {return o;}
	int64_t N4() const {return p;}

	double& operator()(int64_t i, int64_t j, int64_t k, int64_t l)
	{	return operator[](i + j*m + k*(m*n) + l*(m*n*o) );}

	const double& operator()(int64_t i, int64_t j, int64_t k, int64_t l) const
	{	return operator[](i + j*m + k*(m*n) + l*(m*n*o) );}
/*
	void swap( Matrix &y)
	{
