...
// Unwrap p-states
	...
	for (int ev1=1;ev1<=2;ev1++)
	{
		for (int ev2=1;ev2<=2;ev2++)
		{
		...
		complex<double> SpiSmj, SpjSmi; 
		complex<double> SpjSmk, SpkSmj; 
		complex<double> SpkSmi, SpiSmk; 
	...
	//	#pragma omp parallel for default(shared) reduction(+:ch_real, ch_imag, curr_real, curr_imag)
			for(int64_t n1 = 0; n1 < psi1.size(); n1++)
			{
				...
				complex<double> tmp_SpiSmj, tmp_SpjSmi;
				complex<double> tmp_SpjSmk, tmp_SpkSmj;
				complex<double> tmp_SpkSmi, tmp_SpiSmk;
				...
				tmp_SpiSmj  = SipSjm(tri[n].i,  tri[n].j, n1, psi1, psi1, outfile);
				tmp_SpjSmi  = SipSjm(tri[n].j,  tri[n].i, n1, psi1, psi1, outfile);
				tmp_SpjSmk  = SipSjm(tri[n].j,  tri[n].k, n1, psi1, psi1, outfile);
				tmp_SpkSmj  = SipSjm(tri[n].k,  tri[n].j, n1, psi1, psi1, outfile);
				tmp_SpkSmi  = SipSjm(tri[n].k,  tri[n].i, n1, psi1, psi1, outfile);
				tmp_SpiSmk  = SipSjm(tri[n].i,  tri[n].k, n1, psi1, psi1, outfile);
	...
	//	#pragma omp parallel for default(shared) reduction(+:ch_real, ch_imag, curr_real, curr_imag)
			for(int64_t n1 = 0; n1 < psi1.size(); n1++)
			{
				...
				tmp_SpiSmj  = SipSjm( tri[n].i, tri[n].j, n1, psi1, psi3, outfile);
				tmp_SpjSmi  = SipSjm( tri[n].j, tri[n].i, n1, psi1, psi3, outfile);
				tmp_SpjSmk  = SipSjm( tri[n].j, tri[n].k, n1, psi1, psi3, outfile);
				tmp_SpkSmj  = SipSjm( tri[n].k, tri[n].j, n1, psi1, psi3, outfile);
				tmp_SpkSmi  = SipSjm( tri[n].k, tri[n].i, n1, psi1, psi3, outfile);
				tmp_SpiSmk  = SipSjm( tri[n].i, tri[n].k, n1, psi1, psi3, outfile);
	...
	//	#pragma omp parallel for default(shared) reduction(+:ch_real, ch_imag, curr_real, curr_imag)
			for(int64_t n1 = 0; n1 < psi1.size(); n1++)
			{
				...
				tmp_SpiSmj  = SipSjm( tri[n].i, tri[n].j, n1, psi3, psi1, outfile);
				tmp_SpjSmi  = SipSjm( tri[n].j, tri[n].i, n1, psi3, psi1, outfile);
				tmp_SpjSmk  = SipSjm( tri[n].j, tri[n].k, n1, psi3, psi1, outfile);
				tmp_SpkSmj  = SipSjm( tri[n].k, tri[n].j, n1, psi3, psi1, outfile);
				tmp_SpkSmi  = SipSjm( tri[n].k, tri[n].i, n1, psi3, psi1, outfile);
				tmp_SpiSmk  = SipSjm( tri[n].i, tri[n].k, n1, psi3, psi1, outfile);
	...
	//	#pragma omp parallel for default(shared) reduction(+:ch_real, ch_imag, curr_real, curr_imag)
			for(int64_t n1 = 0; n1 < psi1.size(); n1++)
			{
				...
				tmp_SpiSmj  = SipSjm( tri[n].i, tri[n].j, n1, psi3, psi3, outfile);
				tmp_SpjSmi  = SipSjm( tri[n].j, tri[n].i, n1, psi3, psi3, outfile);
				tmp_SpjSmk  = SipSjm( tri[n].j, tri[n].k, n1, psi3, psi3, outfile);
				tmp_SpkSmj  = SipSjm( tri[n].k, tri[n].j, n1, psi3, psi3, outfile);
				tmp_SpkSmi  = SipSjm( tri[n].k, tri[n].i, n1, psi3, psi3, outfile);
				tmp_SpiSmk  = SipSjm( tri[n].i, tri[n].k, n1, psi3, psi3, outfile);
		...
		outfile << "FOR ev1 (ket) = "<< ev1<<" and ev2 (bra) = "<<ev2<<endl;
		outfile << "Expectation value of chirality operator is = " << ch_exp  << endl;	
		...
		outfile << "Expectation value for Si+Sj-                is = " << SpiSmj << endl;	
		outfile << "Expectation value for Sj+Si-                is = " << SpjSmi << endl;	
		outfile << "Expectation value for Sj+Sk-                is = " << SpjSmk<< endl;	
		outfile << "Expectation value for Sk+Sj-                is = " << SpkSmj << endl;	
		outfile << "Expectation value for Sk+Si-                is = " << SpkSmi << endl;	
		outfile << "Expectation value for Si+Sk-                is = " << SpiSmk << endl<<endl;	
		outfile.flush();
		}
	}
