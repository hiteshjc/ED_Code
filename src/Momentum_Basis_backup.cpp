...
void Construct_States(	vector<int64_t> &v_sz,
			...
			double &py,
			int64_t &n_states,
			vector<int64_t> &all_states,
			vector<int64_t> &rep_ids,
			...
			ofstream &outfile)
{
	...
	all_states.resize(num_configs);
	state_lx.resize(num_configs);
	state_ly.resize(num_configs);
	...
	{ 
		...
		double tmp_ph = -px*lx - py*ly;
		state_ph[i] = exp( complex<double>(0.0, tmp_ph) );
	...
	outfile << " Size of " << state_lx.size() << "\t" << state_ly.size() << endl;
}
